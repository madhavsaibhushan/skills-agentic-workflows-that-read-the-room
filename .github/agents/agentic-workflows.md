...
- **CLI commands reference**: For a complete guide on all `gh aw` commands and their MCP tool equivalents (for restricted environments), see `.github/aw/cli-commands.md`
 - **Reference sources**: When relevant, ensure workflows explicitly reference Mona's notes, the GitHub Blog, and the GitHub Changelog as authoritative sources for guidance and change history.
